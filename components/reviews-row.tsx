      ...
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-nowrap items-center gap-3 sm:gap-4 overflow-x-auto no-scrollbar [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {SOURCES.map((item) => (
            <div
              key={item.source}
              className="glass rounded-lg border border-gray-200 px-3.5 py-2.5 flex items-center gap-2.5 min-w-[200px] justify-center shrink-0"
              aria-label={`${item.source} rating ${item.rating} out of 5 from ${item.count}+ reviews`}
            >
              <div className="flex">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star key={i} className={`w-4 h-4 ${i < Math.round(item.rating) ? "fill-yellow-400 text-yellow-400" : "text-gray-300"}`} />
                ))}
              </div>
        ...
        <div className="mt-4 flex gap-4 overflow-x-auto no-scrollbar [-ms-overflow-style:none] [scrollbar-width:none]">
          {[1, 2, 3, 4, 5, 6].map((id) => (
            <div key={id} className="shrink-0 w-80 rounded-lg border border-gray-300 bg-white/90 backdrop-blur-sm shadow-sm p-4">
              ...
              <div className="mt-2 text-[12.5px] text-gray-500">— Evansville Customer</div>
            </div>
          ))}
        </div>
      </div>
