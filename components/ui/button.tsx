...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg font-medium transition-all disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg:not([class*='size-'])]:size-4 shrink-0 [&_svg]:shrink-0 outline-none focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px] aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive border",
  {
    variants: {
      variant: {
        default:
          'bg-primary text-primary-foreground border-border/30 shadow-xs hover:bg-primary/90 hover:brightness-110 hover:border-primary/50 hover:-translate-y-0.5',
        destructive:
        ...
        secondary:
          'bg-secondary text-secondary-foreground border-border/30 shadow-xs hover:bg-secondary/80 hover:brightness-105 hover:border-border/50 hover:-translate-y-0.5',
        ghost:
          'border-transparent hover:bg-accent hover:text-accent-foreground hover:border-border/30 hover:-translate-y-0.5 dark:hover:bg-accent/50',
        link: 'text-primary underline-offset-4 hover:underline border-transparent',
      },
      size: {
        xs: 'h-7 px-2 text-xs has-[>svg]:px-1.5 [&_svg]:size-3',
        sm: 'h-8 px-3 text-xs has-[>svg]:px-2.5 [&_svg]:size-3.5',
        default: 'h-9 px-4 py-2 text-sm has-[>svg]:px-3 [&_svg]:size-4',
        lg: 'h-10 px-6 text-sm md:text-base has-[>svg]:px-4 [&_svg]:size-4',
        xl: 'h-11 px-8 text-base md:text-lg has-[>svg]:px-6 [&_svg]:size-5',
        icon: 'size-9 text-sm [&_svg]:size-4',
        'icon-sm': 'size-8 text-xs [&_svg]:size-3.5',
        'icon-lg': 'size-10 text-base [&_svg]:size-5',
      },
    },
    defaultVariants: {
      variant: 'default',
