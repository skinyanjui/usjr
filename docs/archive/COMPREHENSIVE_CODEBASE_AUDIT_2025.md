...
const price = useMemo(
  () => calculatePrice(),
  [service, loadSize, itemCount, location, urgency]
)
```
...
const prices = activeServices.reduce((acc, s) => {
...
// In component:
{STAR_ICONS.map(i => (
  <Star key={i} className="h-4 w-4 fill-yellow-400" aria-hidden="true" />
))}
```
